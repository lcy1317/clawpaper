...
"""
落先生的文献小窝 - 信任度评估专题
支持 IEEE BibTeX、信任维度、期刊分区和影响因子排序
"""
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .controls { display: justify-content: center; gap: 15px; margin: 20px 0; flex-wrap: wrap; }
        ...
        .badge-download { background: #e74c3c; }
        ...
        .btn-bibtex:hover { transform: scale(1.05); }
...
<body>
    <div class="container">
        ...
        <footer class="footer">
            <p>Made with ♥ for 落先生 🐱</p>
            <p style="opacity:0.8;margin-top:10px">信任是任何分布式系统的核心挑战</p>
        </footer>
    </div>
    ...
    <script>
        ...
        function showModal(paperId) {
            ...
            if (paper.journal_info) {
                let journal = paper.journal_info;
                let impactLabel = journal.impact_factor_label || '';
                let accessUrl = journal.access_url || '';
                let notes = journal.notes || '';
                journalInfoHtml = `
                    <div class="journal-badges" style="margin: 15px 0;">
                    ...
                    <p style="color: #666; margin-bottom: 15px;">
                        <strong>访问链接：</strong>
                        ${accessUrl ? '<a href="' + accessUrl + '" target="_blank" style="color: #667eea;">点击访问</a>' : '无'}
                    </p>
            ...
            if (paper.trust_dimensions) {
                let dims = Object.entries(paper.trust_dimensions).map(([k, v]) => 
            ...
            document.getElementById('modalBody').innerHTML = '
                ...
                <div style="color:#666;margin-bottom:15px;line-height:1.8">
                    ...
                    <strong>机构：</strong>' + paper.institution + '
                </div>
                ' + journalInfoHtml + '
                ' + dimensionsHtml + '
        ...
        function sortPapers(sortType) {
            ...
            cards.sort((a, b) => {
                ...
                if (sortType === 'q1') {
                    const aRank = a.dataset.ranking || '';
                    const bRank = b.dataset.ranking || '';
                    return (bRank === 'SCI Q1' ? 1 : 0) - (aRank === 'SCI Q1' ? 1 : 0);
                }
                if (sortType === 'q2') {
                    const aRank = a.dataset.ranking || '';
                    const bRank = b.dataset.ranking || '';
                    return (aRank === 'SCI Q2' ? 1 : 0) - (bRank === 'SCI Q2' ? 1 : 0);
                }
                if (sortType === 'if_desc') {
...
def generate_papers_html(papers):
    ...
    for paper in papers:
        ...
        btn_download = ''
        if paper.get('file'):
            btn_download = f'<a href="/download/{paper["file"]}" class="btn btn-download">📥 PDF</a>'
        elif downloadable and access_url:
            btn_download = f'<a href="{access_url}" target="_blank" class="btn btn-download">📥 PDF</a>'
        
        btn_access = ''
        if access_url:
            btn_access = f'<a href="{access_url}" target="_blank" class="btn btn-access">🔗 访问</a>'
        
        html_parts.append(f'''
            <article class="paper-card" data-ranking="{ranking}" data-if="{impact}">
                <h3 class="paper-title">{paper['title']}</h3>
                <div class="paper-meta"><strong>{', '.join(paper['authors'])}</strong><br>📅 {paper['year']} | 🏛️ {paper['institution']}</div>
                
                <div class="journal-badges">
                    <span class="badge badge-ranking">{ranking or 'N/A'}</span>
                    <span class="badge badge-type">{paper_type or 'N/A'}</span>
                    {f'<span class="badge badge-if">{impact_label}</span>' if impact_label else ''}
                    <span class="badge badge-publisher">{publisher}</span>
                </div>
                
                <div class="paper-abstract"><strong>摘要：</strong><br>{paper['abstract']}</div>
                ...
                <div class="action-btns">
                    {btn_download}
                    {btn_access}
                    <button class="btn btn-bibtex" onclick="showModal('{paper['id']}')">📋 详情</button>
                </div>
            </article>
        ''')
